...
module.exports = {
    sendMessage(receiverContactNo) {
    return new Promise((resolve, reject) => {
      client.messages.create(
        {
          to: receiverContactNo,
          from: "+12024106618",
          body: `Hi,
                 This is dummy message just to verify twilio service working fine or not`
        },
        (err, message) => {
          if (err) {
            reject(err);
  ...
  twilioPhoneVerification () {
    return new PhoneVerification(process.env.API_KEY, process.env.API_URL)
  }
